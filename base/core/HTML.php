...
class HTML {
    ...
    public static function link($normalpath, $attrib = []) {
        $stringFinal = "<link href='" . URL::to('public') . '/' . $normalpath . "'";
        foreach ($attrib as $key => $value) {
            $stringFinal .= " " . $key . "='" . $value . "'";
        }
        return $stringFinal . ">";
    ...
    public static function file( $normalpath ){
        return URL::to('public'). '/' .$normalpath;
    }
}
